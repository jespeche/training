...
class UserServiceEventTests(@Autowired val userService: UserService) {
    ...
    @Test
    fun `Check event is triggered when a new user is registered`() {
    ...
    @Test
    fun `Demonstrates that an exception pops up in the caller since it is not async`() {
        println("[ Test ] - Testing register")
        whenever(eventHandler.handleUserRegistered(any())).then { throw RuntimeException() }
